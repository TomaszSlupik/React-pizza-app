...
const sizePizza =  (size) => {
	...
	return (
		<div>
			...
			<img className={"sizepizza small " + (base === 1000 ? "checkedpizza" : '')}
			onClick={() => sizePizza(1000)}
			src={process.env.PUBLIC_URL + "/img/pizza.png"} alt="rozmiar pizzy"/>
			<img className={"sizepizza medium " + (base === 1200 ? "checkedpizza": '')} 
			onClick={() => sizePizza(1200)}
			 src={process.env.PUBLIC_URL + "/img/pizza.png"} alt="rozmiar pizzy"/>
			<img className={"sizepizza big " + (base === 1500 ? "checkedpizza": '')} 
				...
				{item.map((itemPizza, index) => {
					return (
						<div key={index} className="layoutitem">
							<input type="checkbox" checked={itemPizza.checked} onChange={()=>itemChange (itemPizza)}></input>
							<img
								className="imgpizza"
